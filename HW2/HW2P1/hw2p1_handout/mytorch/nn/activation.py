...
class GELU:
    ...
    def forward(self, Z):
        self.Z = Z
        self.Phi = Z * 0.5 * (1 + np.erf(Z / np.sqrt(2)))
        self.A = Z * self.Phi
...
class Swish:
    ...
    def backward(self, dLdA):
        sig = self.sigmoid
        dAdZ = sig * (1 + self.beta * self.Z * (1 - sig))

        # Grdient sum over all elements
        self.dLdbeta = np.sum(dLdA * self.Z ** 2 * sig * (1 - sig))
        dLdZ = dLdA * dAdZ
...
class Softmax:
    ...
    def backward(self, dLdA):
        ...
        N = self.A.shape[0]  # Batch size
        C = self.A.shape[1]  # Number of classes
        ...
        for i in range(N):
            ...
            # So for (1×C) @ (C×C) we will get (1×C), and we will store it as row i
