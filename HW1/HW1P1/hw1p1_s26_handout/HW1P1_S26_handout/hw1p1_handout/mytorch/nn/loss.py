...
class MSELoss:
    ...
    def backward(self):
        ...
        Read the writeup (Hint: MSE Loss Section) for implementation details for below code snippet.
        """
        dLdA = (2.0 / (self.N * self.C)) * (self.A - self.Y)
        return dLdA
...
class CrossEntropyLoss:
    def forward(self, A, Y):
        ...
        expZ = np.exp(Z)
        denom = expZ @ Ones_C
        self.softmax = expZ / (denom @ Ones_C.T) # TODO - Can you reuse your own softmax here, if not rewrite the softmax forward logic?

        crossentropy = (-Y * np.log(self.softmax)) @ Ones_C
        sum_crossentropy_loss = (Ones_N.T @ crossentropy)[0, 0]  # scalar
