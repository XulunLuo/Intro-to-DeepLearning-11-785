...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -0.5 * np.sum(dLdNZ * (self.Z  - self.M) * inv_std3, axis=0, keepdims=True) # TODO: Compute gradient of loss backprop through variance calculation.
        sum_centered = np.sum(self.Z - self.M, axis=0, keepdims=True)  
        dNZdM = -inv_std + (1.0 / self.N) * (self.Z - self.M) * inv_std3 * sum_centered  # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ * dNZdM, axis=0, keepdims=True)  # TODO: Compute gradient of loss with respect to mean.

        dLdZ = (dLdNZ * inv_std + dLdV * (2.0 / self.N) * (self.Z - self.M) + (1.0 / self.N) * dLdM)  # TODO: Compute gradient of loss with respect to the input.
