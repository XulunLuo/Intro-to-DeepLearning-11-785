...
from mytorch.nn.layernorm import LayerNorm1d
...
try:
    ...
    with open(f"{base_dir}/solution_outputs/adam_sol_W.pkl", "rb") as stream:
        test_W = pickle.load(stream)

    with open(f"{base_dir}/solution_outputs/adam_sol_b.pkl", "rb") as stream:
...
try:
    ...
    with open(f"{base_dir}/solution_outputs/adamW_sol_W.pkl", "rb") as stream:
        test_W = pickle.load(stream)

    with open(f"{base_dir}/solution_outputs/adamW_sol_b.pkl", "rb") as stream:
        test_b = pickle.load(stream)
...
try:
    ...
    with open(f"{base_dir}/solution_outputs/dropout_sol_forward.pkl", "rb") as stream:
...
try:
    ...
    with open(f"{base_dir}/solution_outputs/dropout_sol_backward.pkl", "rb") as stream:
...
print_header("3.1 Test LayerNorm Forward")
try:
    reset_prng()
    layernorm = LayerNorm1d(num_features=64)
    x = np.random.randn(20, 64).astype(np.float32)
    y = layernorm.forward(x)
    with open(f"{base_dir}/solution_outputs/layernorm_sol_forward.pkl", "rb") as stream:
        test_layernorm_forward = pickle.load(stream)

    TEST_LayerNorm_forward = np.allclose(y, test_layernorm_forward, atol=1e-5)
    print_header("[STUDENT OUTPUT]  LayerNorm Forward")
    print_array_details(y[:4, :4].round(4))
    print_header("[EXPECTED OUTPUT]  LayerNorm Forward")
    print_array_details(test_layernorm_forward[:4, :4].round(4))
except Exception as ex:
    print_header(f"Error in LayerNorm Forward: {str(ex.args)}")
    TEST_LayerNorm_forward = False

print_status_message(
    test_number = "3.1    ",
    check_result = TEST_LayerNorm_forward, 
    description = "LayerNorm Forward"
)

###############################################################################
print_header("3.2 Test LayerNorm Backward")
try:
    reset_prng()
    layernorm = LayerNorm1d(num_features=64)
    x = np.random.randn(20, 64).astype(np.float32)
    y = layernorm.forward(x)
    reset_prng()
    delta = np.random.randn(20, 64).astype(np.float32)
    dx = layernorm.backward(delta)
    with open(f"{base_dir}/solution_outputs/layernorm_sol_backward.pkl", "rb") as stream:
        test_layernorm_backward = pickle.load(stream)

    TEST_LayerNorm_backward = np.allclose(dx, test_layernorm_backward, atol=5e-5)
    print_header("[STUDENT OUTPUT]  LayerNorm Backward")
    grid = dx[:4, :4].round(4)
    print_array_details(grid)
    print_header("[EXPECTED OUTPUT]  LayerNorm Backward")
    print_array_details(test_layernorm_backward[:4].round(4))
except Exception as ex:
    print_header(f"Error in LayerNorm Backward: {str(ex.args)}")
    TEST_LayerNorm_backward = False
print_status_message(
    test_number = "3.2    ",
    check_result = TEST_LayerNorm_backward, 
    description = "LayerNorm Backward"
)

###############################################################################
print_header("3.3 Test LayerNorm dgamma and dbeta")
try:
    reset_prng()
    layernorm = LayerNorm1d(num_features=64)
    x = np.random.randn(20, 64).astype(np.float32)
    ...
    delta = np.random.randn(20, 64).astype(np.float32)
    dx = layernorm.backward(delta)
    dgamma = layernorm.dLdgamma
    dbeta = layernorm.dLdbeta
    with open(f"{base_dir}/solution_outputs/layernorm_sol_dgamma.pkl", "rb") as stream:
        test_layernorm_dgamma = pickle.load(stream)
    with open(f"{base_dir}/solution_outputs/layernorm_sol_dbeta.pkl", "rb") as stream:
        test_layernorm_dbeta = pickle.load(stream)

    TEST_LayerNorm_dgamma = np.allclose(dgamma, test_layernorm_dgamma, atol=1e-5)
    TEST_LayerNorm_dbeta = np.allclose(dbeta, test_layernorm_dbeta, atol=1e-5)
    print_header("[STUDENT OUTPUT]  LayerNorm dgamma")
    print_array_details(dgamma.round(4))
    print_header("[EXPECTED OUTPUT]  LayerNorm dgamma")
    print_array_details(test_layernorm_dgamma.round(4))
    print_header("[STUDENT OUTPUT]  LayerNorm dbeta")
    print_array_details(dbeta.round(4))
    print_header("[EXPECTED OUTPUT]  LayerNorm dbeta")
    print_array_details(test_layernorm_dbeta.round(4))
except Exception as ex:
    print_header(f"Error in LayerNorm dgamma/dbeta: {str(ex.args)}")
    TEST_LayerNorm_dgamma = False
    TEST_LayerNorm_dbeta = False

print_status_message(
    test_number = "3.3    ",
    check_result = TEST_LayerNorm_dgamma & TEST_LayerNorm_dbeta, 
    description = "LayerNorm dgamma and dbeta"
)
...
SCORE_LayerNorm_Forward = 2.5 * int(TEST_LayerNorm_forward)
SCORE_LayerNorm_Backward = 2.5 * int(TEST_LayerNorm_backward)
SCORE_LayerNorm_dgamma_dbeta = 5. * int(TEST_LayerNorm_dgamma & TEST_LayerNorm_dbeta)
SCORE_Total = 30.0
...
GRADE_LOGS = {
    ...
    "LayerNorm Forward": SCORE_LayerNorm_Forward,
    "LayerNorm Backward": SCORE_LayerNorm_Backward,
    "LayerNorm dgamma and dbeta": SCORE_LayerNorm_dgamma_dbeta,
}
